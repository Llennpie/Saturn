...
void apply_twirl_gravity(struct MarioState *m) {
    ...
    m->vel[1] -= 4.0f * heaviness * gravity;
...
void apply_gravity(struct MarioState *m) {
    if (m->action == ACT_TWIRLING && m->vel[1] < 0.0f) {
        ...
        m->vel[1] -= 2.0f * gravity;
        if (m->vel[1] < -37.5f) {
            if ((m->vel[1] += 4.0f * gravity) > -37.5f) {
                m->vel[1] = -37.5f;
            }
        }
    } else {
        m->vel[1] -= 4.0f * gravity;
        if (m->vel[1] < -75.0f) {
            m->vel[1] = -75.0f;
        }
    }
