    ...
    for (size_t i = 0; i < n_vertices; i++, dest_index++) {
        ...
        if (rsp.geometry_mode & G_LIGHTING) {
            ...
            if (cc_model_support || chroma_floor) {
                if (mario_hat) {
                    r = defaultColorHat[0][1];
                    g = defaultColorHat[1][1];
                    b = defaultColorHat[2][1];
                }
                if (mario_overalls) {
                    r = defaultColorOveralls[0][1];
                    g = defaultColorOveralls[1][1];
                    b = defaultColorOveralls[2][1];
                }
                if (mario_gloves) {
                    r = defaultColorGloves[0][1];
                    g = defaultColorGloves[1][1];
                    b = defaultColorGloves[2][1];
                }
                if (mario_shoes) {
                    r = defaultColorShoes[0][1];
                    g = defaultColorShoes[1][1];
                    b = defaultColorShoes[2][1];
                }
                if (mario_skin) {
                    r = defaultColorSkin[0][1];
                    g = defaultColorSkin[1][1];
                    b = defaultColorSkin[2][1];
                }
                if (mario_hair) {
                    r = defaultColorHair[0][1];
                    g = defaultColorHair[1][1];
                    b = defaultColorHair[2][1];
                }
                if (cc_spark_support) {
                    if (mario_shirt) {
                        r = sparkColorShirt[0][1];
                        g = sparkColorShirt[1][1];
                        b = sparkColorShirt[2][1];
                    }
                    if (mario_shoulders) {
                        r = sparkColorShoulders[0][1];
                        g = sparkColorShoulders[1][1];
                        b = sparkColorShoulders[2][1];
                    }
                    if (mario_arms) {
                        r = sparkColorArms[0][1];
                        g = sparkColorArms[1][1];
                        b = sparkColorArms[2][1];
                    }
                    if (mario_overalls_bottom) {
                        r = sparkColorOverallsBottom[0][1];
                        g = sparkColorOverallsBottom[1][1];
                        b = sparkColorOverallsBottom[2][1];
                    }
                    if (mario_legtop) {
                        r = sparkColorLegTop[0][1];
                        g = sparkColorLegTop[1][1];
                        b = sparkColorLegTop[2][1];
                    }
                    if (mario_legbottom) {
                        r = sparkColorLegBottom[0][1];
                        g = sparkColorLegBottom[1][1];
                        b = sparkColorLegBottom[2][1];
                    }
                }
                if (chroma_floor) {
                    r = chromaColor[0][1];
            ...
            for (int i = 0; i < rsp.current_num_lights - 1; i++) {
                ...
                if (intensity > 0.0f) {
                    ...
                    if (cc_model_support || chroma_floor) {
                        if (mario_hat) {
                            r += intensity * defaultColorHat[0][0];
                            g += intensity * defaultColorHat[1][0];
                            b += intensity * defaultColorHat[2][0];
                        }
                        else if (mario_overalls) {
                            r += intensity * defaultColorOveralls[0][0];
                            g += intensity * defaultColorOveralls[1][0];
                            b += intensity * defaultColorOveralls[2][0];
                        }
                        else if (mario_gloves) {
                            r += intensity * defaultColorGloves[0][0];
                            g += intensity * defaultColorGloves[1][0];
                            b += intensity * defaultColorGloves[2][0];
                        }
                        else if (mario_shoes) {
                            r += intensity * defaultColorShoes[0][0];
                            g += intensity * defaultColorShoes[1][0];
                            b += intensity * defaultColorShoes[2][0];
                        }
                        else if (mario_skin) {
                            r += intensity * defaultColorSkin[0][0];
                            g += intensity * defaultColorSkin[1][0];
                            b += intensity * defaultColorSkin[2][0];
                        }
                        else if (mario_hair) {
                            r += intensity * defaultColorHair[0][0];
                            g += intensity * defaultColorHair[1][0];
                            b += intensity * defaultColorHair[2][0];
                        }
                        else if (mario_shirt) {
                            r += intensity * sparkColorShirt[0][0];
                            g += intensity * sparkColorShirt[1][0];
                            b += intensity * sparkColorShirt[2][0];
                        }
                        else if (mario_shoulders) {
                            r += intensity * sparkColorShoulders[0][0];
                            g += intensity * sparkColorShoulders[1][0];
                            b += intensity * sparkColorShoulders[2][0];
                        }
                        else if (mario_arms) {
                            r += intensity * sparkColorArms[0][0];
                            g += intensity * sparkColorArms[1][0];
                            b += intensity * sparkColorArms[2][0];
                        }
                        else if (mario_overalls_bottom) {
                            r += intensity * sparkColorOverallsBottom[0][0];
                            g += intensity * sparkColorOverallsBottom[1][0];
                            b += intensity * sparkColorOverallsBottom[2][0];
                        }
                        else if (mario_legtop) {
                            r += intensity * sparkColorLegTop[0][0];
                            g += intensity * sparkColorLegTop[1][0];
                            b += intensity * sparkColorLegTop[2][0];
                        }
                        else if (mario_legbottom) {
                            r += intensity * sparkColorLegBottom[0][0];
                            g += intensity * sparkColorLegBottom[1][0];
                            b += intensity * sparkColorLegBottom[2][0];
                        }
                        else if (chroma_floor) {
                            r += intensity * chromaColor[0][0];
                            g += intensity * chromaColor[1][0];
                            b += intensity * chromaColor[2][0];
                        }
                        else {
                            r += intensity * lightr;
                            g += intensity * lightg;
